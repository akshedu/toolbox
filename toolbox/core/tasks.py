...
def create_top_videos():
    for timerange in settings.TIMERANGE_DICT.keys():
        ...
        for index, row in video_incremental_df.iterrows():
            to_create.appned(TopVideos(video_id=row['video_id'],
                                       metric=row['metric'],
                                       date=end_date,
                                       frequency=timerange,
...
def create_top_channels():
    for timerange in settings.TIMERANGE_DICT.keys():
        ...
        channel_incremental_df = pd.DataFrame(list(video_incremental))
        ...
        to_create = []
        for index, row in video_incremental_df.iterrows():
            to_create.appned(TopVideos(video_id=row['channel_id'],
                                       metric=row['metric'],
                                       date=end_date,
                                       frequency=timerange,
                                       incremental=row['incremental']))
