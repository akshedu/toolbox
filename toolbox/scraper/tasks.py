...
from toolbox.scraper.utils import create_youtube_service, chunkify, chunks, get_channel_list, get_video_list
...
def srape_youtube_channels():
    try:
        youtube_service_list = [create_youtube_service(settings.YOUTUBE_SCOPES, file) for file in settings.SERVICE_ACCOUNT_FILES]
    ...
    for i in range(settings.TRACKED_CHANNEL_SPLITS):
        scrape_youtube_channel_chunks.apply_async(youtube_service_list[i], channel_chunks[i])

@shared_task
def scrape_youtube_channel_chunks(youtube_service, channel_list):
    ...
    if not channel_details_missing == []:
        for channel_chunks in chunks(channel_details_missing, settings.YOUTUBE_RESOURCE_LIST_LIMIT):
            channel_details_task(youtube_service, channel_list, CHANNEL_DETAILS_PART)
    ...
    for channel_chunks in chunks(channel_list, settings.YOUTUBE_RESOURCE_LIST_LIMIT):
        channel_stats_task(youtube_service, channel_list, RESOURCE_STATISTICS_PART)
    ...
    for item in channel_query_response.get('items', []):
        channel_stats_to_create.append(
            ChannelStats(
                ...
                subscriber=item.get('statistics', {}).get('subscriberCount', None),
...
@shared_task
def channel_details_task(youtube_service, chanel_list, part):
    channel_query_response = get_channel_list(youtube_service, channel_list, part)
    channels_to_create = []
    for item in channel_query_response.get('items', []):
        channels_to_create.append(
            Channel(
                channel_id=item.get('id', None),
                description=Description.obejcts.create(
                ...
                thumbnail=Thumbnail.objects.create(
                    default_url=item.get('snippet', {}).get('thumbnail', {}).get('default', {}).get('url', None),
                    medium_url=item.get('snippet', {}).get('thumbnail', {}).get('medium', {}).get('url', None),
                    high_url=item.get('snippet', {}).get('thumbnail', {}).get('high', {}).get('url', None)),
